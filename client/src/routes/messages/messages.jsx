...
export default function Messages() {
    // !! Next job: personalize ui (change from static data)
    // TODO: update ui from received data
    // While loading use loading screen
    // While loading, fetch data from server
    ...
    const handleNewChat = async (feed) => {
        if (feed) {
            // Create feed
            const feed_name = document.getElementById("feed-name-input").value;
            const feed_desc = document.getElementById("feed-desc-input").value;
            const feed_discoverable = document.getElementById(
                "feed-discoverable-0"
            ).checked;
            const feed_dissappearing_msg = document.getElementById(
                "feed-dissappearing-msg-0"
            ).checked;
            const feed_passw =
            ...
            const req = await fetch("http://api.ernestsgm.com/api/chat/new", {
                ...
                body: JSON.stringify({
                    ...
                    discoverable: feed_discoverable ? 1 : 0,
                    disappearing: feed_dissappearing_msg ? 100 : 0,
                    password: feed_passw,
                }),
            });
    ...
    React.useEffect(() => {
        document.addEventListener("click", (e) => {
            if (settings && !e.target.closest(".btn.active")) {
                setSettings(false);
            }
            if (more && !e.target.closest(".btn.active")) {
                setMore(false);
            }
        });
    }, [settings, more]);
    ...
    const postMessage = async () => {
        ...
        if (!req.ok) {
            console.log("[ERROR] Could not post message");
            return;
        }
    ...
    return (
        <div className="messages-cont">
            <Nav active={2} msg_count={3} friends_count={2} />
            <main className="messages">
                <aside className="sidebar">
                    <header className="sidebar-header">
                        <h1>Messages</h1>
                        <Button
                            text={<h2 className="secondary">+</h2>}
                            id="add-btn"
                            narrow
                            onClick={() => {
                    ...
                    <div className="sidebar-outer">
                        <section className="feeds sidebar-sect">
                            ...
                            <p className="section-item secondary">
                                <span className="list">#</span> Pacifica
                            </p>
                            <p className="section-item secondary">
                                <span className="list">#</span> NCPD Gigs
                            </p>
                            <p className="bold section-item secondary">
                                        ...
                                        <NavLink
                                            className={
                                                parseInt(chat.unread_count) > 0
                                                    ? "section-item secondary bold"
                                                    : "section-item secondary"
                                            }
                                            id={chat.chat_id}
                                            to={"/app/msg/" + chat.chat_id}
                                            key={chat.chat_id}
                                        >
                                            <span className="list">#</span>{" "}
                                            {chat.chat_name}
                                            {parseInt(chat.unread_count) >
                                                0 && (
                        ...
                        <section className="direct sidebar-sect">
                            <h3 className="secondary bold section-header section-item">
                                Direct
                            </h3>
                            <ul>
                                ...
                                <li className="section-item secondary bold">
                                    <span className="list status idle" />
                                    Person 2
                ...
                <article className="main">
                    <header className="main-head">
                        <div>
                            <p className="primary pixel-font" id="fingerprint">
                        ...
                        <div className="btns">
                            ...
                            <div
                                className={settings ? "btn active" : "btn"}
                            ...
                            <div
                                className={more ? "btn active" : "btn"}
                                onClick={() => toggleDropdowns("more")}
                            >
                                ...
                                <div className="dropdown">
                                    <ul>
                                        ...
                                        <li>Hide users</li>
                                        <li className="primary">Report</li>
                                        <li className="primary">Leave chat</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    ...
                    <section className="input-cont">
                        <Button
                            text={<h2 className="secondary light">+</h2>}
                            id="add-file"
                            narrow
                        />
                ...
                <aside className="users">
                    <header className="users-head">
                        <h1>Users</h1>
                        <p className="small pixel-font primary small-xtra">
                            UserList initialized
                        </p>
                        <p className="small pixel-font primary one-line small-xtra">
                            NetWire Seed: _7dghggvegfgs678gfsdf
                        </p>
                    </header>
                    <hr className="hr" />
                    <ul className="user-list">
                        <li className="user secondary section-item">
                            <span className="list status offline" />
                            Ben Dover
                        </li>
                        <li className="user secondary section-item">
            ...
            <div className="add-overlay">
                <div className="add-popup">
                    ...
                    <div className="tab-group">
                        ...
                        />
                        <label htmlFor="add-chat-tab">Chat</label>
                        <input
                            type="radio"
                            name="add-chat-tab"
                            id="add-feed-tab"
                        ...
                        />
                        <label htmlFor="add-feed-tab">Feed</label>
                    </div>
                    <div className="tab" id="add-chat">
                        <Input
                            label="Display name"
                    ...
                    <div className="tab" id="add-feed">
                        <Input label="Feed name" id="feed-name-input" />
                        <Input label="Feed description" id="feed-desc-input" />
                        <Input
                            label="Password"
                            id="feed-passw-input"
                            type="password"
                    ...
                    <Button
                        ...
                        onClick={() =>
                            handleNewChat(
                                document.getElementById("add-feed-tab").checked
                            )
                        }
                    />
                </div>
